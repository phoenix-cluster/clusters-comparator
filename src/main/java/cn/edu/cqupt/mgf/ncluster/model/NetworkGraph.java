...
public class NetworkGraph {
    // input parameters
    private UndirectedGraph<MSVertex, MSEdge> undirectedGraph;
    ...
    private HashMap<MSVertex, Integer> layer; // vertex => the layer number of the vertex
    private HashMap<MSVertex, Double> startYOfVerticesShape; // initial value: the Y coordinate of the upper-left corner

    // shape
    private int totalLayerNumber; // total layer number
    private HashMap<MSVertex, Rectangle> verticesShape;
    ...
    public NetworkGraph(String releaseIName, String releaseIIName,
                        UndirectedGraph<MSVertex, MSEdge> undirectedGraph,
                        MSVertex startVertex) {
        ...
        buildsFactor = new SimpleDoubleProperty(100.0);
        ...
        ArrayList<MSVertex> bfsVerticesOrder = undirectedGraph.getBfsVerticesOrder(startVertex);
     ...
     */
    private void arrangeLayer(ArrayList<MSVertex> bfsVerticesOrder) {
        int i = -1; // layer number : 0-base system
        String preReleaseName = null;
        for (MSVertex vertex : bfsVerticesOrder) {
     ...
     * @return
     */
    private double[] getEachLayerLength(HashMap<MSVertex, Integer> layer) {
        double[] eachLayerLength = new double[totalLayerNumber];
        for (MSVertex vertex : layer.keySet()) {
    ...
    private void setVerticesShape(HashMap<MSVertex, Integer> layer,
                                  double[] eachLayerLength,
     ...
     */
    private void setVertexEvent(MSVertex vertex, Rectangle rect) {
...
//        });
    ...
    private void setEdgesShape(UndirectedGraph<MSVertex, MSEdge> undirectedGraph,
                               HashMap<MSVertex, Integer> layer,
                               HashMap<MSVertex, Rectangle> verticesShape,
                               double verticesShapeWidth) {
        // HashMap<Vertex, HashMap<Vertex, Edge>> unredundantAdjacencyTable =
        // undirectedGraph
        // .getUnredundantAdjacencyTable();
        HashMap<MSVertex, HashMap<MSVertex, MSEdge>> unredundantAdjacencyTable =
                undirectedGraph.getUnredundantAdjacencyTable();
        int i = 0;
        for (MSVertex vertex1 : unredundantAdjacencyTable.keySet()) {
            for (MSVertex vertex2 : unredundantAdjacencyTable.get(vertex1).keySet()) {
                MSEdge edge = undirectedGraph.getEdge(vertex1, vertex2); // get edge
    ...
    private void setEdgeEvent(MSVertex vertex1, MSVertex vertex2,
                              MSEdge edge, Path path) {
