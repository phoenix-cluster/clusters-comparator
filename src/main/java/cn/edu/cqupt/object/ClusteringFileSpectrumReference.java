...
public class ClusteringFileSpectrumReference implements ISpectrumReference {
	...
	public ClusteringFileSpectrumReference(String sequence, int charge, float precursorMz, String id,
			float similarityScore, String species, String modifications) {
	...
	public ClusteringFileSpectrumReference(String specLine) throws Exception {
		...
		if (fields.length > INDEX_CHARGE)
			charge = (int) Math.round(Double.parseDouble(fields[INDEX_CHARGE]));
		else
			charge = 0;
	...
	/**
	 * Creates the PSM objects based on the sequence string and the modification
	 * strings.
	 */
	private void createPSMs() {
		...
		if (modificationsPerPSM != null && sequences.length != modificationsPerPSM.length) {
			throw new IllegalStateException(
					"Different number of peptide sequences and modification definitions encountered.");
		}
	...
	public final class Peak {
		...
		public boolean equals(Object o) {
			...
			if (Float.compare(peak.intensity, intensity) != 0)
				return false;
			if (Float.compare(peak.mz, mz) != 0)
				return false;
	...
	public boolean equals(Object o) {
		...
		if (charge != that.charge)
			return false;
		if (hasPeaks != that.hasPeaks)
			return false;
		if (isIdentified != that.isIdentified)
			return false;
		if (Float.compare(that.precursorMz, precursorMz) != 0)
			return false;
		if (Float.compare(that.similarityScore, similarityScore) != 0)
			return false;
		...
		if (mostCommonPsm != null ? !mostCommonPsm.equals(that.mostCommonPsm) : that.mostCommonPsm != null)
			return false;
		if (peaks != null ? !peaks.equals(that.peaks) : that.peaks != null)
			return false;
		if (psms != null ? !psms.equals(that.psms) : that.psms != null)
			return false;
