...
public class SankeyDiagram {
    ...
    // the width of vertex
    private SimpleDoubleProperty verticesWidth;

    // vertex => shape
    private HashMap<Vertex, Rectangle> verticesShapes;
    ...
    private HashMap<Vertex, Double> startYOfVertices;
    ...
    public SankeyDiagram(UndirectedGraph<Vertex, Edge> undirectedGraph,
        ...
        verticesWidth = new SimpleDoubleProperty(10.0);
        firstRunFlag = true;

        // initialize
        verticesShapes = new HashMap<>();
        startYOfVertices = new HashMap<>();
        ...
        // set edges shape
        setEdgesShape(this.undirectedGraph, layer, verticesShapes, verticesShapeWidth.get());

        // set controller
        setController();

        // add edges shape
        addEdgesShape(edgesShapes);

        // add vertices shape
        addVerticesShape(verticesShapes.values());
     ...
     */
    private void arrangeLayers(UndirectedGraph<Vertex, Edge> undirectedGraph) {
        Set<Vertex> allVertices = undirectedGraph.getAllVertices();
        for(int i = 0; i < allVertices.size(); i++){

        }
        undirectedGraph.getAllVertices().parallelStream()
                .forEach(vertex -> layers.put(vertex, vertex.getLayer()));
        totalLayerNumber = layers.; // set total layer number
    }
    ...
    private void setVerticesShape(HashMap<Vertex, Integer> layer, double[] eachLayerLength,
        ...
        for (Vertex vertex : layer.keySet()) {
            ...
            // create vertex shape
            Rectangle rect = firstRunFlag ? new Rectangle() : verticesShapes.get(vertex);
            ...
            // rect.setFill(null);
            if (firstRunFlag) {
                verticesShapes.put(vertex, rect);
                startYOfVertices.put(vertex, rect.getY());
            } else {

                // update startY of vertices shape in not first run
                startYOfVertices.replace(vertex, rect.getY());
            }
     ...
     *
     * @param vertex
     * @param rect   the rectangle of corresponding vertex
     */
    ...
    private void setEdgesShape(UndirectedGraph<Vertex, Edge> undirectedGraph, HashMap<Vertex, Integer> layer,
                               HashMap<Vertex, Rectangle> verticesShapes, double verticesShapeWidth) {
        ...
        for (Vertex vertex1 : unredundantAdjacencyTable.keySet()) {
            for (Vertex vertex2 : unredundantAdjacencyTable.get(vertex1).keySet()) {
                ...
                if (layer.get(vertex1) < layer.get(vertex2)) {
                    ...
                    // vertex1
                    startX1 = verticesShapes.get(vertex1).getX() + verticesShapeWidth;
                    startY1 = startYOfVertices.get(vertex1);
                    endX1 = startX1;
                    endY1 = startY1 + height;

                    // vertex2
                    startX2 = verticesShapes.get(vertex2).getX();
                    startY2 = startYOfVertices.get(vertex2);
                    endX2 = startX2;
                    endY2 = startY2 + height;
                } else {

                    // vertex1
                    startX1 = verticesShapes.get(vertex2).getX() + verticesShapeWidth;
                    startY1 = startYOfVertices.get(vertex2);
                    endX1 = startX1;
                    endY1 = startY1 + height;

                    // vertex2
                    startX2 = verticesShapes.get(vertex1).getX();
                    startY2 = startYOfVertices.get(vertex1);
                    endX2 = startX2;
                    endY2 = startY2 + height;
                }
                ...
                // update startY
                startYOfVertices.replace(vertex1, startYOfVertices.get(vertex1) + height);
                startYOfVertices.replace(vertex2, startYOfVertices.get(vertex2) + height);
                ...
                if (firstRunFlag) {
                    ...
                    edgesShapes.add(path);

                    // set edge event
                    setEdgeEvent(vertex1, vertex2, edge, path);
                } else {
                    Path path = edgesShapes.get(i++);
...
//			overlapCountLab.setText("");
    ...
    private void addEdgesShape(ArrayList<Path> edgesShapes) {
        group.getChildren().addAll(edgesShapes);
    }
    ...
    private GridPane setController() {
        ...
        verticesShapeWidthController.valueProperty().addListener(new ChangeListener<Number>() {
            ...
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                ...
                // set edges shape
                setEdgesShape(undirectedGraph, layer, verticesShapes, verticesShapeWidth.get());
            }
        });
